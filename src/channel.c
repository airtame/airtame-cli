...
int channel_init(Metrics_t *metrics, Channel_t *channel) {
    int i;
    ...
    if (metrics) {
        ...
        metrics->net_bytes_received = 0;
        for(i=0; i<NUM_PACKET_TYPES; i++) {
            metrics->packet_summary[i] = 0;
        }
        metrics->net_latency_sum = 0;
        metrics->net_latency_cnt = 0;
        metrics->net_avg_latency = 0;
    }
...
int channel_multicast_send(Channel_t *channel, char *address, int port, char *buff, int size) {
    ...
    for(i=0;i<channel->used_fds;i++) {
        ...
        if (channel->metrics) channel->metrics->net_bytes_sent += size;
    }
    return rc;
}
...
int channel_recv(Channel_t *channel, char *buff, int size) {
    ...
    if (channel->metrics) channel->metrics->net_bytes_received += rc;
    return rc;
}
